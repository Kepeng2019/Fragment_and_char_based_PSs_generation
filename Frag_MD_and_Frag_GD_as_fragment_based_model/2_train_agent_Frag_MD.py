...
def train_agent(epoch, restore_prior_from='./data/Frag_ML/Prior_RNN_frag_ML.ckpt',
    ...
    for step in range(n_steps):
        ...
        if step+1 >= n_steps:
            ...
            os.makeidrs('./data/Frag_ML_agent_function1', exist_ok=True)
            save_smiles_df.to_csv('./data/Frag_ML_agent_function1/' + 'epoch_'+str(epoch) +'_smiles.csv', index=False, header=False)
            pd.DataFrame(score_list).to_csv('./data/Frag_ML_agent_function1/' + 'epoch_'+str(epoch) +'_scores.csv', index=False, header=False)
            break
