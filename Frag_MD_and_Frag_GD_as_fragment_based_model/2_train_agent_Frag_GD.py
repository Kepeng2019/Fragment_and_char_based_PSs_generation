...
from MCMG_utils.utils import Variable, fraction_valid_smiles, unique, seq_to_smiles_frag
...
def train_agent(epoch, restore_prior_from='./data/Frag_GL/Prior_RNN_frag_GL_epoch_10.ckpt',
    ...
    for step in range(n_steps):
        ...
        score1,score2 = get_scoring_function('st_abs_f1')(smiles)
        ...
        if step+1 >= n_steps:
            ...
            os.makedirs('./data/Frag_GL_agent_function1', exist_ok=True)
            save_smiles_df.to_csv('./data/Frag_GL_agent_function1/' + 'epoch_'+str(epoch) +'_smiles.csv', index=False, header=False)
            pd.DataFrame(score_list).to_csv('./data/Frag_GL_agent_function1/' + 'epoch_'+str(epoch) +'_scores.csv', index=False, header=False)
            break
