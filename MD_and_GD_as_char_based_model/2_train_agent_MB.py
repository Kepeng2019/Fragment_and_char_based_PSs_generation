...
def train_agent(epoch,restore_prior_from='./data/ML/Prior_RNN_ML.ckpt',
    ...
    logger = VizardLog('./data/Agent_ML_function2_logs')
    ...
    for step in range(n_steps):
        ...
        if step+1 >= n_steps:
            ...
            save_smiles_df.to_csv('./data/Agent_ML_function2_smiles/' + 'epoch_'+str(epoch) +'_smiles.csv', index=False, header=False)
            pd.DataFrame(score_list).to_csv('./data/Agent_ML_function2_smiles/' + 'epoch_'+str(epoch) +'_scores.csv', index=False, header=False)
