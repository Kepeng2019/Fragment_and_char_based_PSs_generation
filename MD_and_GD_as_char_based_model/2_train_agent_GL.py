...
def train_agent(epoch,restore_prior_from='./data/GL/Prior_RNN_GL.ckpt',
                restore_agent_from='./data/GL/Prior_RNN_GL.ckpt',agent_save='./',
                batch_size=128, n_steps=5001, sigma=60, save_dir='./',
                experience_replay=0):
    voc = Vocabulary(init_from_file="./data/voc.csv")

    logger = VizardLog('./data/GL_RL_logs')
    ...
    for step in range(n_steps):
        ...
        score1,score2 = get_scoring_function('st_abs')(smiles)  # modified
        ...
        if step+1 >= n_steps:
            ...
            save_smiles_df.to_csv('./data/GL_RL_smiles/' + 'epoch_'+str(epoch) +'_smiles.csv', index=False, header=False)
            pd.DataFrame(score_list).to_csv('./data/GL_RL_smiles/' + 'epoch_'+str(epoch) +'_scores.csv', index=False, header=False)
            torch.save(Prior.rnn.state_dict(), './data/GL_RL/RNN_GL_{epoch}.ckpt')
            break
