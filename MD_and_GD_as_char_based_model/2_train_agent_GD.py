...
def train_agent(epoch,restore_prior_from='./data/GL/Prior_RNN_GL.ckpt',
    ...
    os.makedirs('./data/Agent_GL_function1_logs',exist_ok=True)
    logger = VizardLog('./data/Agent_GL_function1_logs')
    ...
    for step in range(n_steps):
        ...
        if step+1 >= n_steps:
            ...
            os.makedirs('./data/Agent_GL_function1_smiles',exist_ok=True)
            save_smiles_df.to_csv('./data/Agent_GL_function1_smiles/' + 'epoch_'+str(epoch) +'_smiles.csv', index=False, header=False)
            pd.DataFrame(score_list).to_csv('./data/Agent_GL_function1_smiles/' + 'epoch_'+str(epoch) +'_scores.csv', index=False, header=False)
